...
const addItems = (state) => {
  if (state.diff.added.length > 0) {
    return meta(state).then(doDownloads);
  }
  ...
  if (state.diff.removed.length > 0) {
    state.diff.removed.forEach((item) => {
      try {
        ...
        unlinkSync(join(state.repoPath, "assets", imgPath));
      } catch (err) {}
    });
  }
  return state;
...
const downloadImg = async (filepath, url) => {
  try {
    ...
    console.error(
      "Download Error: " + url + " to " + filepath + "(" + e.message + ")",
    );
    return null;
  }
};

const doDownloads = async (state) => {
  for (const key of state.diff.added) {
    ...
    await downloadImg(imgAbsPath, srcUrl);
  }
  return state;
};
