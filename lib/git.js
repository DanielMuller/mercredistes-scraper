...
module.exports.clone = () => {
  execSync(`git clone --quiet https://${gitRepositoryURL} ${path.join('/tmp', GITHUB_REPO)} 2> /dev/null || (cd ${path.join('/tmp', GITHUB_REPO)} && git pull)`)
...
module.exports.push = () => {
  process.chdir(`${path.join('/tmp', GITHUB_REPO)}`)
  const gitStatus = execSync(`git cherry -v`).toString()
  if (gitStatus) {
